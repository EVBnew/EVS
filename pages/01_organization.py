...
with right:
    ...
    if submitted:
        try:
            if not temp_pwd or len(temp_pwd) < 4:
                ...
                create_user(email=email, role=role_new, password=temp_pwd, status=status, created_by=user["email"])
    ...
    if not emails:
        ...
        email_sel = st.selectbox("Utilisateur", options=sorted(emails))
        new_pwd = st.text_input("Nouveau mot de passe", type="password", placeholder="ex: demo1234")
    ...
    if emails:
        email_sel2 = st.selectbox("Utilisateur (statut)", options=sorted(emails), key="status_email")
        new_status = st.selectbox("Nouveau statut", options=["active", "inactive", "pending"], index=0)
